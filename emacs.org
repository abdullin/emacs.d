...
In order to load everything, copy =build/init.el= (shown below) to
=~/.emacs.d/= folder and enable the roles that you are interested in.

#+BEGIN_SRC elisp :tangle build/init.el
  (setq ra/roles '(
                   "org"
                   ;; "ess"
                   ;; "erlang"
